...
@section('content-body')
    <div class="mt-1 p-2 card">
        ...
        <div class="col-8">
            <form action="{{ route('payee.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="payee_name" class="form-label">Payee Name</label>
                    <input type="text" class="form-control" id="payee_name" name="payee_name" 
                           value="{{ old('payee_name') }}" placeholder="Payee Name" tabindex="1" autofocus>
                    @error('payee_name')
                ...
                <div class="mb-3">
                    ...
                    <input type="account_number" class="form-control" id="account_number" name="account_number" 
                           value="{{ old('account_number') }}" placeholder="Account Number" tabindex="3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="phone" name="phone" 
                           value="{{ old('phone') }}" placeholder="Phone Number" tabindex="2" maxlength="11">
                    @error('phone')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control" id="email" name="email" 
                           value="{{ old('email') }}" placeholder="Email Address" tabindex="3">
                    @error('email')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
